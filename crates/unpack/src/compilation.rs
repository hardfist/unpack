...
use crate::{
    ...
    errors::Diagnostics,
    module::{CodeGenerationContext, CodeGenerationResult, ModuleGraph, ModuleId, ModuleScanner, ScannerState},
};
use std::sync::Arc;
...
impl Compilation {
    ...
    pub fn code_generation(&self, linker_state: LinkerState) -> CodeGenerationState {
        ...
        for (id, result) in results {
            ...
            code_generation_results.module_id_to_generation_result.insert(id, result.expect("codegeneration failed"));
        };
        CodeGenerationState {
    ...
    // chunk asset
    pub fn create_chunk_asset(&self, code_generation_state: &mut CodeGenerationState) -> ChunkAssetState {
        // let manifest = vec![];
        let mut assets = FxHashMap::default();
        for chunk_id in code_generation_state.chunk_graph.chunks.indices() {
            let chunk_name = code_generation_state.chunk_graph.chunk_by_id(chunk_id).name.to_owned().expect("should have name");
            let chunk_modules = code_generation_state.chunk_graph.get_chunk_modules(chunk_id);
            dbg!(&chunk_modules);
            let chunk_source = self.render_chunk_modules(code_generation_state,chunk_id, chunk_modules);
    ...
    pub fn render_chunk_modules(&self,state: &mut CodeGenerationState, chunk_id: ChunkId, chunk_modules: IndexSet<ModuleId>) -> BoxSource{
        let module_sources = chunk_modules.iter().map(|module_id| {
            dbg!(self.render_module(state, chunk_id, *module_id))
        });
        let concat_source = ConcatSource::new(module_sources);
        concat_source.boxed()
    }
    pub fn render_module(&self, state: &mut CodeGenerationState,_chunk_id: ChunkId,module_id: ModuleId) -> BoxSource{
        state.code_generation_results.module_id_to_generation_result[&module_id].source.clone()
    }
}
